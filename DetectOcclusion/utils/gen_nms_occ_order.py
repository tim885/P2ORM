...
if dataset == 'ibims':
    ...
    res_root_dir = os.path.join(curr_dir, '..', 'experiments/output/iBims1OR_pretrained/results_vis/test_179_{}'.format(dataset))
elif dataset == 'nyuv2':
...
for idx, big_idx in enumerate(tqdm(test_ids)):
    ...
    order_in_path  = os.path.join(res_order_dir, '{}-order-pix.npz'.format(big_idx))
    ...
    occ_order = np.load(order_in_path)['order']
